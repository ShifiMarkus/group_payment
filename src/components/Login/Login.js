...
import { setCurrentUserId, setCurrentUserName } from "../store/user";
    ...
    if (fields.email && fields.password) {
      ...
      try{
        const res = await fetch(
          `https://localhost:44320/api/users/${fields.email}/${fields.password}`
        );
        debugger
        if (res.ok) {
          // setLogin(true)
          ...
          dispatch(setCurrentUserId(currentUser.userCode));
          dispatch(setCurrentUserName(currentUser.firstName));
          if (checked) {
            setCookies('name', data.firstName, { path: '/' });
          }
